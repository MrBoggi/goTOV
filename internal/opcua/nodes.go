...
func (c *Client) ListSymbols(ctx context.Context) ([]*ua.NodeID, error) {
	...
	for _, s := range symbols {
		...
		if err != nil {
			c.log.Debug().Err(err).Msgf("⚠️ Could not read DisplayName for %s (using tag as fallback)", s)
			c.SetDisplayName(id.String(), s)
		} else if len(attrs) > 0 {
			val := attrs[0].Value.Value()
			if lt, ok := val.(*ua.LocalizedText); ok {
				c.SetDisplayName(id.String(), lt.Text)
				c.log.Debug().Msgf("🏷  %s → DisplayName = %s", s, lt.Text)
			} else {
				c.SetDisplayName(id.String(), s)
